...
async def promote(promt):
    ...
    try:
        ...
        await promt.edit("`Jadi Babu Berhasil!`")
        await sleep(5)
        await promt.delete()
...
async def demote(dmod):
    ...
    rank = "Anak Pantek"  # muahahahaha kena demote, mampus.
    ...
    await dmod.edit("`Kasian Bukan Admin Lagi!`")
    await sleep(5)
    await dmod.delete()
...
async def ungmoot(un_gmute):
    ...
    await un_gmute.edit("```Ungmute dulu...```")

    if ungmute(user.id) is False:
        await un_gmute.edit("`Error! User probably not gmuted.`")
    else:
        # Inform about success
        await un_gmute.edit("```Laen Kali Jangan tengil```")
        await sleep(3)
...
async def gspider(gspdr):
    ...
    if gmute(user.id) is False:
        ...
        if reason:
            await gspdr.edit(f"`Berisik Pantekkk!`\nReason: {reason}")
        else:
            await gspdr.edit("`Diam dulu Pantekkkk!`")
